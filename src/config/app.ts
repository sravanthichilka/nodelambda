...
import { APP_ENVIRONMENT } from "../helper/constants";
const Joi = require("joi");

let dotEnvConfig: any = dotenv.config({ debug: true });
...
function validateEnvConfiguration() {
  ...
  if (dotEnvConfig.parsed.APP_ENVIRONMENT === APP_ENVIRONMENT.LOCAL) {
  ...
  const isFiniteNumberPresignedPostSecond = isFinite(
    dotEnvConfig.parsed.S3_CREATE_PRESIGNED_POST_SECOND
  );

  const isFiniteNumberCloudfrontGetExpireTimeInSecond = isFinite(
    dotEnvConfig.parsed.CLOUDFRONT_GET_EXPIRE_TIME_IN_SECOND
  );

  if (isFiniteNumber || isFiniteNumberPresignedPostSecond) {
    dotEnvConfig.parsed.JWT_REFRESH_EXPIRES_IN = parseInt(
      dotEnvConfig.parsed.JWT_REFRESH_EXPIRES_IN
    );
    dotEnvConfig.parsed.S3_CREATE_PRESIGNED_POST_SECOND = parseInt(
      dotEnvConfig.parsed.S3_CREATE_PRESIGNED_POST_SECOND
    );
  } else {
  ...
  if (isFiniteNumberCloudfrontGetExpireTimeInSecond) {
    dotEnvConfig.parsed.CLOUDFRONT_GET_EXPIRE_TIME_IN_SECOND = parseInt(
      dotEnvConfig.parsed.CLOUDFRONT_GET_EXPIRE_TIME_IN_SECOND
    );
  } else {
  ...
  if (isDBMONGOTLSINSECOND) {
    dotEnvConfig.parsed.DB_MONGO_TLS_IN_SECOND = parseInt(
      dotEnvConfig.parsed.DB_MONGO_TLS_IN_SECOND
    );
  } else {
...
export default {
  ...
  jwt: {
    ...
    REFRESH_EXPIRES_IN: parseInt(<string>process.env.JWT_REFRESH_EXPIRES_IN),
    RESET_PASSWORD_EXPIRES_IN: process.env.JWT_RESET_PASSWORD_TOKEN_EXPIRES_IN,
  },
  S3: {
    CLOUDFRONT_URL_TEST: <string>process.env.S3_CLOUDFRONT_URL_TEST,
    AWS_CLOUDFRONT_ACCESSKEY: <string>process.env.S3_AWS_CLOUDFRONT_ACCESSKEY,
    AWS_CLOUDFRONT_PK: <string>process.env.S3_AWS_CLOUDFRONT_PK,
    AWS_BUCKET_NAME: <string>process.env.S3_AWS_BUCKET_NAME,
    S3_CREATE_PRESIGNED_POST_SECOND: <string>process.env.S3_CREATE_PRESIGNED_POST_SECOND,
    CLOUDFRONT_GET_EXPIRE_TIME_IN_SECOND: parseInt(
      <string>process.env.CLOUDFRONT_GET_EXPIRE_TIME_IN_SECOND
    ),
  },
  google: {
    RECAPTCHA_SECRET: <string>process.env.GOOGLE_RECAPTCHA_SECRET,
  },
  MAIL: {
    SES_AWS_REGION: <string>process.env.SES_AWS_REGION,
    SES_SENDER_EMAIL: <string>process.env.SES_SENDER_EMAIL,
    SES_SECRET_KEY: <string>process.env.SES_SECRET_KEY,
    SES_ACCESS_KEY: <string>process.env.SES_ACCESS_KEY,
    SES_USERNAME: process.env.SES_USERNAME,
    SES_PASSWORD: process.env.SES_PASSWORD,
  },
  MONGO_DB: {
    DB_MONGO_DB: process.env.DB_MONGO_DB,
    DB_MONGO_TLS_IN_SECOND: parseInt(<string>process.env.DB_MONGO_TLS_IN_SECOND),
  },
};
