...
import customerRepo from './documenttypes.repo';
import userRepo from '../users/users.repo';
import HelperController from '../auth/auth.helper'
import log from '../../helper/logs';
import { ENUM_DOCUMENT_TYPE_STATUS } from "../../helper/constants";
import {
...
class CustomerController extends HelperController {
    ...
    async fetchDocumentTypes(req: any, res: any) {
        try {

            const { user: { id: consumeUserId, role: consumeUserRole },
                params: { }
            } = req;
            ...
            sqlQueryParams.push(ENUM_DOCUMENT_TYPE_STATUS.ACTIVE);


            let sqlQueryBuilder = sqlQuery.join(" and ");
            let sqlQueryBuilderList = sqlQueryParams;

            let documentTypesList = await customerRepo.getDocumentTypeFetchList(sqlQueryBuilder, sqlQueryBuilderList, { sortObject, pageLimit: pageLimitObj });
